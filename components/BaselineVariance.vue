...
const formatVariance=v=>v.toFixed(6).replace(/0{1,2}$/, '')
const sumEquation=computed(()=>String.raw`\operatorname{Var}(\hat g_b)=`+result.value.outcomes.map(o=>Number(o.varianceTerm.toFixed(9)).toString()).join('+')+'='+formatVariance(result.value.variance))
const comparison=computed(()=>result.value.variance<.9375-1e-9?'Variance below 0.9375 at this fixed policy.':result.value.variance>.9375+1e-9?'Variance above 0.9375 at this fixed policy.':'Same variance as the uncentered update.')
</script>
<template>
<div class="baseline-variance" @click.stop>
  ...
  <tbody><tr v-for="(o,i) in result.outcomes" :key="i"><td>{{o.label}}</td><td>{{o.probability.toFixed(3)}}</td><td>{{o.raw.toFixed(3)}}</td><td :data-centered="i" class="centered">{{o.gradient.toFixed(3)}}</td><td :data-variance-term="i">{{Number(o.varianceTerm.toFixed(9))}}</td></tr></tbody></table>
  <div class="variance-calculation" data-variance-calculation><MathInline :tex="sumEquation" /></div>
  <div class="variance-summary"><div><h3>Mean gradient</h3><strong data-gradient-mean>{{result.mean.toFixed(2)}}</strong><p>Unchanged for every b.</p></div><div><h3>Variance = sum of last column</h3><div class="variance-bar"><span>Without baseline</span><div class="track"><div style="width:46.875%" class="original"></div></div><b>0.9375</b></div><div class="variance-bar"><span>With this baseline</span><div class="track"><div :style="{width:(result.variance/2*100)+'%'}"></div></div><b data-gradient-variance>{{formatVariance(result.variance)}}</b></div></div></div>
